...
That's not the end of it, though — Cloudflare also runs bot-management checks independent of the TLS layer, and deep-linking straight to certain pages (`table.html`, `results.html`, `home/search.html`) gets a `403` even from genuine Chromium, reproducible in a normal browser too. Two things were needed to get past it:

1. **The real Chrome binary, not `chromium-headless-shell`.** Playwright's default `Headless = true` launches a separate, stripped-down binary with a distinct fingerprint. `Channel = "chromium"` opts into Chromium's "new" headless mode instead — the actual Chrome binary running windowless — which Cloudflare doesn't flag the way it flags the shell.
2. **A cookie-accept-then-reload dance on a 403.** On a `403`, `PlaywrightPageFetcher` opens a throwaway tab to the apex domain (`fulltime.thefa.com`), genuinely clicks the cookie-consent banner, then **reloads** (not re-navigates) the tab that hit the block. A fresh navigation after accepting cookies does not work — it has to be a reload of a tab that already attempted the URL once.

### Fetching details

- **One shared browser.** `PlaywrightPageFetcher` is registered as a singleton and launches Chromium lazily on the first request (guarded by a `SemaphoreSlim`), so the browser start-up cost is paid once rather than per request. It also checks the browser is still connected before reusing it, relaunching if Chromium has crashed. Each fetch then gets its own fresh `BrowserContext` — an isolated cookie/cache jar — which is disposed as soon as the page has been read.
